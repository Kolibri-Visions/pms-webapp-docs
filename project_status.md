...
- **BUGFIX: Booking Requests Approve Idempotency Heal (2026-01-29, updated)**: Fixed approve endpoint idempotency to heal inconsistent state in TWO scenarios. **Symptom**: PROD smoke failing at Test 3 - approve returns 409 (idempotent), but subsequent GET shows status=requested instead of confirmed. **Root Cause (Scenario 1)**: Booking has `confirmed_at` set but `status` != "confirmed". **Root Cause (Scenario 2)**: Booking has active `inventory_ranges` entry (from booking_service activation) but `status` is still "requested". **Solution**: Added TWO healing checks in approve endpoint (backend/app/api/routes/booking_requests.py): (1) Lines 592-621: Detect confirmed_at set but status != confirmed → heal status to confirmed, return 200 idempotent. (2) Lines 653-690: Detect inventory_ranges entry with kind='booking', source_id=booking_id, state='active' → heal status to confirmed, return 200 "healed from inventory". Both ensure: Approve is truly idempotent even with stale state, subsequent GET returns confirmed status, list with status=requested excludes effectively-approved requests. **Regression Tests**: (1) test_approve_heals_inconsistent_state - confirmed_at set but status=requested, (2) test_approve_heals_from_inventory_ranges_evidence - inventory_ranges active but status=requested. Both verify approve returns 200, GET shows confirmed. Files modified: backend/app/api/routes/booking_requests.py (healing logic ~70 lines total), backend/tests/integration/test_booking_requests_approve.py (2 regression tests ~120 lines), backend/docs/ops/runbook/03-auth.md (2 troubleshooting sections), backend/docs/project_status.md (this entry). Status: ✅ IMPLEMENTED (awaiting PROD verification: pms_booking_requests_approve_decline_smoke.sh rc=0 with 6/6 tests passed).
