...
├── 00-golden-paths.md      (36 lines)   - Quick-reference happy-path procedures
├── 01-deployment.md       (1173 lines)  - Deploy gating, Coolify, rollback, TLS
├── 02-database.md         (1690 lines)  - DB DNS, schema drift, migrations, data hygiene
├── 03-auth.md              (247 lines)  - JWT token validation, apikey header, CORS
├── 04-channel-manager.md  (2230 lines)  - Redis/Celery setup, sync ops, worker config
└── Total: 5376 lines of focused, navigable content
```
...
- **Deploy Verification**: `backend/scripts/pms_verify_deploy.sh EXPECT_COMMIT=eb624c8` → rc=0 (commit match; both backend and admin verification passed)
...
- **Deploy Verification**: `backend/scripts/pms_verify_deploy.sh EXPECT_COMMIT=06a6cf7` → rc=0 (commit match; both backend and admin verification passed)
...
9. **Admin UI — Periods Management (2026-01-16)**:
...
Re-verified 2026-01-15 via `backend/scripts/pms_p2_full_smoke.sh` (rc=0) on commit `eb624c8`. Deploy verification: `backend/scripts/pms_verify_deploy.sh EXPECT_COMMIT=eb624c8` → rc=0 (commit match; both backend and admin verification passed). See the P2 Pricing v1 Foundation re-verification block above for full details.
...
- Updated actions: Bearbeiten | Archivieren | Löschen
...
- Preview shows: "N Saisonzeiten würden erstellt"
...
- "Saison hinzufügen" button to add new seasons

- Per-season fields: Label, Start Date, End Date, Price (EUR)

- Real-time overlap validation with visual feedback

- Error message for overlaps: "Überschneidung mit bestehender Saison"
...
- Warning message: "Lücken gefunden: DD.MM.YYYY - DD.MM.YYYY, ..."
...
- "Löschen" action added to row actions menu

- Confirmation dialog: "Dieser Preisplan wird gelöscht. Fortfahren?"
...
- Error message: "Kann nicht löschen: Preisplan wird von aktiven Buchungen verwendet"
...
# 2. Navigate to: Objekte → [Select Property] → "Preispläne" tab
...
# - Expected: Preview shows "N Saisonzeiten würden erstellt"
...
# - Expected: Error "Überschneidung mit bestehender Saison"
...
# - Warning visible: "Nur für Notfälle verwenden..."
...
# - If dependencies: Error "Kann nicht löschen: Preisplan wird von aktiven Buchungen verwendet"
...
1. **Gaps Blocking Save**: Expected behavior, add more seasons or enable fallback → See "Gaps Blocking Save" in runbook

2. **Multiple Active Plans**: UI should prevent, check DB for is_archived=false count → See "Multiple Active Plans" in runbook

3. **Fallback Toggle Not Showing**: Accordion collapsed, expand "Erweitert" → See "Fallback Toggle Not Showing" in runbook

4. **Delete Fails**: Plan has dependent bookings/quotes, use archive instead → See "Delete Fails" in runbook

5. **Template Selector Empty**: No active templates, create at /pricing/seasons → See "Template Selector Empty" in runbook

**Detailed Troubleshooting**: See [P2.11 Objekt-Preispläne Troubleshooting](../docs/ops/runbook.md#p211-objekt-preispläne-saisonpflicht--vorlagencustom-editor) in runbook.md

### Related Documentation

- [P2.11 Runbook](../docs/ops/runbook.md#p211-objekt-preispläne-saisonpflicht--vorlagencustom-editor) - Operations guide with verification steps and troubleshooting
...
- ✓ Preislogik column shows season count and fallback suffix

- ✓ "Als Standard" action removed from table

- ✓ One active plan rule enforced with confirmation dialog

- ✓ Edit modal template mode: Select template, set prices, preview, apply

- ✓ Edit modal custom mode: Add/remove seasons, overlap validation

- ✓ Gap checking blocks save with specific date ranges
...
**Properties Dropdown Fix (2026-01-23):** ✅ VERIFIED

- **Status**: ✅ VERIFIED (2026-01-23, commit 83ace7c)
...
- **PROD Evidence (2026-01-23)**:
  ...
  - **Backend Version**: `GET /api/v1/ops/version` → `service=pms-backend, source_commit=83ace7cc66797f55605ad70561a3ce3df9c0a76e, started_at=2026-01-23T19:24:05.437898+00:00`

  - **Admin Version**: `GET /api/ops/version` → `service=pms-admin, source_commit=83ace7cc66797f55605ad70561a3ce3df9c0a76e, started_at=2026-01-23T19:21:51.461Z`
...
## P2.16 Smoke Script Hardening — Season Template Sync (PROD 409 Fix)
...
- PROD environments already have active plans → HTTP 409 "Für dieses Objekt ist bereits ein aktiver Preisplan vorhanden"
...
1. ✅ Smoke script `rc=0` in PROD (all 4 tests pass)

2. ✅ Deployed commit matches git commit hash (via `/api/v1/ops/version`)
...
- **P2.21.4.5 (2026-01-26)**: Admin Property Media UX improvements and real storage deletion. (1) Overview cover image: Moved to FIRST card (before Listed Status), reduced size (max-h-40, compact recognition image), changed to object-contain (no cropping, gray letterbox background), maintains retry logic + cache-bust. (2) DELETE /api/v1/properties/{id}/media/{id} now deletes from Supabase Storage: service fetches storage_provider + storage_path before delete, calls storage.delete_file() if provider is "supabase", then soft deletes DB record. Idempotent (missing file = success). Returns 409 if storage delete fails (actionable message). (3) Media gallery lightbox: Thumbnails now clickable, opens modal with enlarged image (max 85vh, object-contain), "Öffnen in neuem Tab" + "Schließen" buttons, same signed URL + retry logic. (4) Smoke script Test 3.5: After delete, verifies display_url returns HTTP 404/410 (not 200/206), retries 3x with 1s delay for eventual consistency. Total tests: 6→7. Runbook updated with media delete storage troubleshooting + lightbox usage. Status: ✅ VERIFIED. Production Evidence (2026-01-26): Deploy verification (pms_verify_deploy.sh rc=0): Backend /api/v1/ops/version source_commit=97ec65db0aaee225a9a2033dd7315e160a8e6ae3 started_at=2026-01-26T10:45:04.690536+00:00, Admin /api/ops/version source_commit=97ec65db0aaee225a9a2033dd7315e160a8e6ae3 started_at=2026-01-26T10:42:48.463Z. Smoke verification (pms_property_media_upload_smoke.sh rc=0, 7/7 tests passed): signed display_url reachable (HTTP 206), cover toggle works, deletion removes storage file (Test 3.5: "Deleted file not accessible", HTTP 400).
- **P2.21.4.6 (2026-01-26)**: Admin UI Overview + Media Lightbox UX improvements. (1) Media lightbox click fix: Fixed pointer-events bug where hover overlay blocked thumbnail clicks - overlay now has `pointer-events-none`, action buttons have `pointer-events-auto`, clicking anywhere on thumbnail opens lightbox. (2) Overview redesign: Hero summary section combining cover preview (left, aspect-video, max-w 400px, object-contain) + key facts (right: status, listed, guests, bedrooms, base price, min stay) + quick actions (Medien verwalten, Listen/Unlisten, Eigentümer). Content cards reorganized with emoji icons (📋 Objektinformationen with description+type, 📍 Adresse & Lage, 🛏️ Kapazität, 💰 Zeiten & Preise, 🔑 IDs less prominent, ⏱️ Zeitstempel less prominent, 🗺️ Koordinaten, ⭐ Ausstattung). (3) Edit modal enhancements: Added data-testid (`overview-edit-toggle`, `edit-modal-overlay`), ARIA attributes (role="dialog", aria-labelledby). Existing edit functionality preserved (all fields editable: name, internal_name, description, max_guests, base_price, cleaning_fee, min_stay, is_active, address fields, capacity, times, prices). (4) Smoke script: New bash script `pms_admin_ui_overview_media_smoke.sh` validates HTML structure - checks data-testid presence for lightbox, cover section, edit toggle. (5) Documentation: Runbook sections for lightbox pointer-events fix + overview redesign verification, scripts README updated with new smoke test docs. Status: ✅ IMPLEMENTED (awaiting PROD verification: pms_verify_deploy.sh rc=0 + pms_admin_ui_overview_media_smoke.sh rc=0 3/3 tests + manual UI verification - lightbox clickable, hero section first, edit modal works, no regressions).
- **P2.21.4.7 (2026-01-26)**: Admin UI Polish - Lightbox Navigation + Overview Reordering + Coordinates Editing. (1) Lightbox prev/next navigation: Added left/right arrow controls inside lightbox modal (‹ and › buttons), keyboard support (ArrowLeft/ArrowRight to navigate, ESC to close), wrap-around navigation (last→first, first→last), arrows hidden when 0-1 images, index indicator (e.g., "2 / 5") shown when multiple images. Implemented with lightboxIndex state, openLightbox/goToPrevious/goToNext functions, keyboard event listener. data-testids: lightbox-prev, lightbox-next, lightbox-index. (2) Overview card reordering: Moved ⭐ Ausstattung and 🗺️ Koordinaten & Karte cards UP to appear directly after 💰 Zeiten & Preise (before 🔑 IDs and ⏱️ Zeitstempel). Final order: Hero → Objektinformationen → Adresse → Kapazität → Zeiten & Preise → Ausstattung → Koordinaten & Karte → IDs → Zeitstempel. (3) Coordinates editing: Added Breitengrad (latitude) and Längengrad (longitude) inputs to edit modal, with validation (lat: -90 to 90, lng: -180 to 180, step 0.000001, empty allowed → null), data-testids (edit-lat, edit-lng). Updated handleStartEdit to include lat/lng. Backend already supports latitude/longitude via PropertyUpdate schema (ge=-90, le=90, ge=-180, le=180). Saves via existing PATCH /api/v1/properties/{id} endpoint. Added data-testids to coordinate display (overview-lat, overview-lng). (4) Smoke script updated: 3→6 tests, added Test 4 (lightbox-prev/next), Test 5 (overview-lat/lng), Test 6 (edit-lat/lng). (5) Documentation: Updated scripts/README.md with 6/6 test descriptions + expected output. Status: IMPLEMENTED (awaiting PROD verification: pms_verify_deploy.sh rc=0 + pms_admin_ui_overview_media_smoke.sh rc=0 6/6 tests + manual UI verification - lightbox arrows work, keyboard nav works, card order correct, coordinates editable and saved).
...
- **BUGFIX: Booking Requests Approve Idempotency Heal (2026-01-29)**: Fixed approve endpoint idempotency to heal inconsistent state. **Symptom**: PROD smoke failing at Test 3 - approve returns 409 (idempotent), but subsequent GET shows status=requested instead of confirmed. **Root Cause**: When a booking request has `confirmed_at` set (was previously approved) but `status` is still "requested" (stale state from partial/incomplete approval), the approve endpoint: (1) Line 593 idempotent check failed because status != "confirmed", (2) Line 608 check failed because status was "requested" but state was inconsistent, (3) Exclusion constraint could fire causing 409. The status was never healed to match the confirmed_at evidence. **Solution**: Added healing logic in approve endpoint (backend/app/api/routes/booking_requests.py lines 649-680) that detects inconsistent state (confirmed_at set but status != confirmed), updates status to "confirmed", and returns 200 with message "Booking request already approved (idempotent, state healed)". This ensures: (1) Approve is truly idempotent even with stale state, (2) Subsequent GET returns correct confirmed status, (3) List with status=requested no longer includes effectively-approved requests. **Regression Test**: Added test_approve_heals_inconsistent_state in backend/tests/integration/test_booking_requests_approve.py - creates booking with confirmed_at set but status=requested, verifies approve returns 200 with healed state, verifies subsequent GET shows confirmed. Files modified: backend/app/api/routes/booking_requests.py (healing logic ~30 lines), backend/tests/integration/test_booking_requests_approve.py (regression test ~60 lines), backend/docs/ops/runbook/03-auth.md (troubleshooting update), backend/docs/project_status.md (this entry). Status: ✅ IMPLEMENTED (awaiting PROD verification: pms_booking_requests_approve_decline_smoke.sh rc=0 with 6/6 tests passed).
