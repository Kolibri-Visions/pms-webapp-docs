...
- **QA P1 (2026-01-28)**: Smoke output standardization (v1). Scope: Standardize output format across 3 smoke scripts for consistent parsing and CI integration. Changes: (1) Output format - Changed from mixed "PASSED"/"FAILED"/emoji patterns to consistent `[PASS]`/`[FAIL]` prefix on all test result lines. (2) Exit codes - Standardized env var guards to exit with code 2 (was exit 1 in some scripts). Exit code semantics: 0=success, 1=test-failure, 2=env-missing. (3) Summary line - All scripts already had "Test Results: X/Y passed" format, verified present. Scripts updated: pms_booking_requests_export_smoke.sh (bash, 2 tests), pms_admin_ui_booking_requests_smoke.sh (Playwright wrapper, 11 tests), pms_admin_ui_properties_list_smoke.sh (Playwright wrapper, 9 tests). No shared helper added per ANTI-KREISREGEL (minimal change, don't over-engineer). Files modified: backend/scripts/pms_booking_requests_export_smoke.sh, backend/scripts/pms_admin_ui_booking_requests_smoke.sh, backend/scripts/pms_admin_ui_properties_list_smoke.sh. Documentation: runbook.md (P1 smoke output format standard section), project_status.md (this entry). QA proofs: /tmp/p1_smoke_output_qa_proofs.txt. Status: ✅ IMPLEMENTED.
